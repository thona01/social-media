...
const FriendController = {
    addUnfriend: async (req, res) => {
        try {
            ...
            const current_user = await User || Admin.findById(current_user_id);
            const friend = await User || Admin.findById(friend_id);
    ...
    getAllFriendAndNotFriend: async (req, res) => {
        try {
            const {id} = req.params;
            const user = await User || Admin.findById(id);
            const friends = [];
            await Promise.all(
            ...
            const all_user = await User || Admin.find();
            const not_friends = [];
            await Promise.all(
                all_user.map(async (single_user) => {
                    if (!user.friends.includes(single_user.id)) {
                        const not_friend = await User || Admin.findById(single_user.id);
